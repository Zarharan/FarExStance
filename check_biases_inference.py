...
file_handler = logging.FileHandler('check_biases_inference.log')
...
model_save_directory= "models/claim_bias"

trainer.test_dataloader = test_dataloader
epoch_model_name= "/epoch30_loss1.493914246559143.pt"
...
df_test_set.to_excel(f"{result_save_path}{epoch_model_name.replace('.pt', '')}_claim_bias_result_on_{test_set_path.split('/')[-1]}")
...
model_save_directory= "models/news_article_bias"

trainer.test_dataloader = test_dataloader
epoch_model_name= "/epoch8_loss1.4956090450286865.pt"
loss, result_metrics, pred_labels= trainer.test_model(model_save_directory + epoch_model_name)
...
df_test_set.to_excel(f"{result_save_path}{epoch_model_name.replace('.pt', '')}_body_bias_result_on_{test_set_path.split('/')[-1]}")
